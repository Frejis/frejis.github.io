...
The front page for the other eight projects, plus a writeup for each one
...
`index.html` is the hub: a card for each of the eight demos with a
...
that links here and to the eight demos directly, so serving or opening the repo
...
- **Not a blog engine.** Eight hand-written HTML files with no feed, no index
  data, no tags page and no build step. Adding a ninth post means writing a
  ninth file and adding a card.
